...
class PreloadedDataset(Dataset):
    def __init__(self, main_dir, shape, transform=None, shuffle=False, use_tqdm=True, augment=False):
        ...
        self.images = torch.zeros(shape)
        self.targets = torch.zeros(0).type(torch.LongTensor)
    ...
    def from_tensors(data, targets, transform=None, device="cpu", use_tqdm=True, augment=False):
    ...
    def __getitem__(self, idx):
        if hasattr(self, 'transformed_images'):
    ...
    def device(self):
        assert self.images.device == self.targets.device, "All images, targets, and transformed images must be on the same device"
        if hasattr(self, 'transformed_images'):
            assert self.images.device == self.transformed_images.device, "All images, targets, and transformed images must be on the same device"
